...
interface TextAreaProps<T extends FieldValues> {
    ...
    rows?: number;
    maxLength?: number;
    showFromDetails?: boolean;
    className?: string;
    textAreaClassName?: string;
}

function TextArea<T extends FieldValues>({ name, control, label, placeholder, caption, rows, maxLength, showFromDetails = true, className, textAreaClassName }: TextAreaProps<T>) {
    const {
        field,
        fieldState: { error },
