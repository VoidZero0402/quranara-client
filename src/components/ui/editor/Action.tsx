...
function Action({ children, label = "", isActive, disabled, onClick }: ActionProps) {
    return (
        <div className="flex-center relative">
            <button onClick={onClick} disabled={disabled} className={`flex-center peer size-10 rounded-xl transition-all ${isActive ? "text-white bg-blue-500" : "text-gray-500 dark:text-gray-200 bg-gray-200 dark:bg-gray-800 disabled:bg-gray-300 dark:disabled:bg-gray-700"}`}>
                {children}
            </button>
