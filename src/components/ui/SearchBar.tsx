...
function SearchBar({ route, query, wrapperCalssName, className, id, placeholder }: SearchBarProps) {
    ...
    return (
        <form onSubmit={onSubmit} className={cn("relative", wrapperCalssName)}>
            <input name="search" type="text" placeholder={placeholder} id={id} className={cn("py-3 px-5 w-full h-12 bg-gray-100 dark:bg-gray-500/10 text-sm text-gray-600 dark:text-gray-200 border border-transparent focus:border-gray-200 dark:focus:border-gray-700 rounded-2xl placeholder:text-gray-500 dark:placeholder:text-gray-400 transition-colors", className)} />
