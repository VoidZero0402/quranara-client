...
import { TYPES, UploadType } from "@/constants/uploads";
...
function Uploader({ isUploaded, onUpload, onCancel }: UploaderProps) {
    ...
    return (
        <>
            {isUploaded ? (
                <div className="flex-center gap-x-4 p-4 h-14 bg-teal-50 dark:bg-teal-500/10 text-teal-500 rounded-2xl">
                ...
                <div>
                    ...
                    {isUploading ? (
                        <div className="flex-center gap-x-1 py-3 px-5 h-14 font-pelak-medium blue-light rounded-2xl cursor-pointer select-none transition-colors">
                            <Spinner show />
                            در حال آپلود فایل
                        </div>
                    ) : (
                        <label htmlFor="uploader" className="flex-center gap-x-1 py-3 px-5 h-14 font-pelak-medium text-gray-600 dark:text-gray-200 bg-gray-100 dark:bg-gray-500/10 hover:bg-gray-200 dark:hover:bg-gray-500/15 rounded-2xl cursor-pointer select-none transition-colors">
                            <Upload />
                            آپلود فایل پیوست
                        </label>
                    )}
