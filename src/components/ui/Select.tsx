...
function Select({ children, value = "", defaultText = "", onSelect, label, placeholder = "لطفا یک گزینه را انتخاب کنید", caption, className, selectClassName }: SelectProps) {
    ...
    return (
        <div ref={ref} className={cn("flex flex-col gap-y-2 relative font-pelak-medium", className)}>
            ...
            <div className={cn("absolute top-full flex flex-col gap-y-1 p-2 w-full max-h-64 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/50 rounded-xl overflow-auto with-custom-scroll invisible opacity-0 mt-4 transition-all duration-300", selectClassName, isOpen && "visible opacity-100 mt-2")}>
