...
function OffCanvas({ children, isOpen, onClose, className, scrollable = false }: OffCanvasProps) {
    ...
    return createPortal(
        <div className={cn("fixed left-0 top-0 bottom-0 size-full z-20 invisible", isOpen && "visible")}>
            ...
            <div className={cn("w-full sm:w-4/5 h-full absolute -right-full sm:-right-[80%] p-4 sm:p-8 space-y-4 bg-white dark:bg-gray-850 transition-all duration-500", scrollable && "overflow-auto with-custom-scroll", isOpen && "-right-0 sm:-right-0", className)}>
